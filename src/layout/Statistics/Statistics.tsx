...
import { DisciplineShort, GroupShort } from '@entities/scheduleRequest';
...
import {
  ...
  buildStatusGradient,
  buildStudentDetail,
  buildStudentTopMetrics,
  buildSubmissionOverviewFromEntries,
  buildWeeklyActivity,
  formatGrade,
  getLatestReportsByTask,
  isStudentRole,
  isStudentTaskOverdue,
  isTeacherRole,
  parseRouteId,
} from './utils/utils';
...
export const Statistics = () => {
  ...
  const [selectedDiscipline, setSelectedDiscipline] =
    useState<DisciplineShort | null>(null);
  ...
  const [groupStudentReports, setGroupStudentReports] = useState<
    StudentReportsEntry[]
  >([]);
  const [selectedStudent, setSelectedStudent] = useState<StudentShort | null>(
    null
  );
  ...
  const loadData = async () => {
    ...
    try {
      if (studentView) {
        ...
        const disciplines = await studentApi.getDisciplinesByGroup(
          profile.groupId
        );
  ...
  const handleDisciplineChange = async (
    ...
    try {
        ...
        Promise.all(
          groups.map(async (group) => {
            ...
            return schedule.some(
              (item) => item.disciplineName === discipline.name
            )
              ? group
              : null;
          })
      ...
      const disciplineGroupsData = scheduleGroups.filter(
        Boolean
      ) as GroupShort[];
  ...
  const selectedStudentDisciplineReports = useMemo(() => {
    ...
    return selectedStudentReports.filter((report) =>
  ...
  const disciplineTopStudents = useMemo(
    () =>
      buildStudentTopMetrics(disciplineStudentReports, disciplineTasks).slice(
        0,
        5
      ),
  ...
  if (!studentView && !teacherView && !isLoading) {
    return (
      <div className={s.statistics}>
        <Alert variant="info">
          Статистика доступна студентам, старостам, преподавателям и
          администратору.
        </Alert>
      </div>
    );
        ...
        <>
          ...
          <section className={s.dashboardGrid}>
            ...
            <article className={`${s.panel} ${s.widePanel}`}>
              ...
              {!studentState ||
              buildDisciplineAverages(studentState.tasks, studentState.reports)
                .length === 0 ? (
                    ...
                    <div className={s.disciplineRow} key={item.disciplineId}>
                      ...
                      <div className={s.barTrack}>
                        <span
                          className={s.barFill}
                          style={{
                            width: `${Math.min(item.averageGrade * 10, 100)}%`,
                          }}
                        />
                      </div>
            ...
            <article className={s.panel}>
              ...
              <div className={s.studyGrid}>
                ...
                <div className={s.studyItem}>
                  ...
                  <strong>
                    {studentState?.notifications.filter((item) => !item.seen)
                      .length ?? 0}
                  </strong>
                </div>
                <div className={s.studyItem}>
            ...
            <>
              ...
              <section className={s.dashboardGrid}>
                  ...
                  <div className={s.donutLayout}>
                    <div
                      ...
                      style={{
                        background: buildStatusGradient({
                          accepted:
                            teacherState.analytics?.submissionOverview
                              .submitted ?? 0,
                          pending:
                ...
                <article className={`${s.panel} ${s.widePanel}`}>
                  ...
                  {!teacherState.analytics ||
                  teacherState.analytics.disciplineAverageGrades.length ===
                    0 ? (
                    <EmptyState text="Оценок по дисциплинам пока нет" />
                  ) : (
                    <div className={s.disciplineList}>
                      {teacherState.analytics.disciplineAverageGrades.map(
                        (item) => (
                          <div
                            className={s.disciplineRow}
                            key={item.disciplineId}
                          >
                            <div className={s.disciplineInfo}>
                              <strong>{item.disciplineName}</strong>
                              <span>
                                {item.gradedReports} проверенных работ
                              </span>
                            </div>
                            <div className={s.barTrack}>
                              <span
                                className={s.barFill}
                                style={{
                                  width: `${Math.max(item.averageGrade, 4)}%`,
                                }}
                              />
                            </div>
                            <b>{formatGrade(item.averageGrade)}</b>
                ...
                <article className={s.panel}>
                  ...
                  teacherState.analytics.topStudents.length === 0 ? (
                    ...
                    <div className={s.topList}>
                      {teacherState.analytics.topStudents.map(
                        (student, index) => (
                ...
                <article className={s.panel} id="admin-summary">
                  ...
                  <div className={s.donutLayout}>
                    <div
                      className={s.donut}
                      style={{
                        background: buildStatusGradient(disciplineOverview),
                      }}
                    >
                    ...
                    <div className={s.legend}>
                      ...
                      <span>
                        ...
                        : {disciplineOverview.overdue}
                      </span>
                      <span>
                        <i className={s.mutedDot} />
                        {
                          '\u0411\u0435\u0437 \u0441\u0434\u0430\u0447\u0438'
                        }: {disciplineOverview.untouched}
                      </span>
                    </div>
                  </div>
                </article>
            ...
            <>
              <section className={s.metricGrid}>
                ...
                <MetricCard
                  ...
                  value={
                    groupMetric ? formatGrade(groupMetric.averageGrade) : '-'
                  }
                  tone="red"
                />
              </section>
                  ...
                  <div className={s.donutLayout}>
                    ...
                    <div className={s.legend}>
                      ...
                      <span>
                        ...
                        : {groupOverview.overdue}
                      </span>
                      <span>
                        <i className={s.mutedDot} />
                        {
                          '\u0411\u0435\u0437 \u0441\u0434\u0430\u0447\u0438'
                        }: {groupOverview.untouched}
                      </span>
                    </div>
                  </div>
                </article>
                  ...
                  <div className={s.donutLayout}>
                    ...
                    <div className={s.legend}>
                      ...
                      <span>
                        ...
                        : {studentDetails.overdue}
                      </span>
                      <span>
                        <i className={s.mutedDot} />
                        {
                          '\u0411\u0435\u0437 \u0441\u0434\u0430\u0447\u0438'
                        }: {studentDetails.untouched}
                      </span>
                    </div>
                  </div>
                </article>
                    ...
                    <div className={s.disciplineRow}>
                      ...
                      <div className={s.barTrack}>
                        <span
                          className={s.barFill}
                          style={{
                            width: `${Math.min(studentDetails.averageGrade * 10, 100)}%`,
                          }}
                        />
                      </div>
